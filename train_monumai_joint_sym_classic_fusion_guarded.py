...
from MonuMAI.symbolic_utils import get_axioms_attr_guarded, SatAgg, LogitsToPredicateResnetTree, LogitsToPredicateMulticlass,LogitsToPredicateMultilabel

def run_epoch(resnet,P_classifier, optimizer, loader, total_loss_meter,ccentropy_loss_meter, sym_loss_metter, acc_meter, sat_meter, criterion, attr_criterion, args, is_training):
    """
    For the rest of the networks (X -> A, cotraining, simple finetune)
    """
    if is_training:
        for p in [resnet,P_classifier]:
            p.train()
    else:
        for p in [resnet,P_classifier]:
            p.eval()   
    
    sdt = P_classifier.model.logits_model
    for batch_idx, data in enumerate(loader):           
        ...
        P_classifier.model.reset_penalty()
        optimizer.zero_grad()        
        ...
        axioms_attr_joint = get_axioms_attr_guarded(attr_outputs, is_training,P_classifier)    
        ############################################
        losses = []
        ...
        penalty_sym =  P_classifier.model.get_penalty()  if is_training else 0.0
...
def train(resnet,sdt, args):   
    ...
    optimizer = torch.optim.Adam([{'params':resnet.parameters()},{'params':P_classifier.model.parameters()}],
                                 lr= args.lr ,
                                 weight_decay=5e-4 )
    ...
    for epoch in range(0, args.epochs):
        ...
        if args.no_img:
            ...
            train_total_loss_meter,train_ccentropy_loss_meter, train_sym_loss_metter, train_sat_meter, train_acc_meter = run_epoch(resnet,P_classifier, optimizer, train_loader, train_total_loss_meter, train_ccentropy_loss_meter, train_sym_loss_metter, train_acc_meter, train_sat_meter, criterion, attr_criterion, args, is_training=True)
       ...
       # evaluate on val set
        ...
        with torch.no_grad():
            val_loss_meter,val_ccentropy_loss_meter, val_sym_loss_metter,val_sat_meter, val_acc_meter = run_epoch(resnet,P_classifier, optimizer, val_loader, val_loss_meter, val_ccentropy_loss_meter, val_sym_loss_metter, val_acc_meter, val_sat_meter, criterion, attr_criterion, args, is_training=False)
        ...
        if best_val_acc < val_acc_meter.avg:
            ...
            torch.save(resnet, os.path.join(args.log_dir, 'best_model_resnet%d.pth' % args.seed))
            torch.save(sdt, os.path.join(args.log_dir, 'best_model_sdt%d.pth' % args.seed))
        if epoch > 20 and best_val_sat < val_sat_meter.avg and best_val_acc < (val_acc_meter.avg + 0.5):
            best_sat_epoch = epoch
            best_val_sat = val_sat_meter.avg
            logger.write('New model best sat model at epoch %d\n' % epoch)
            torch.save(resnet, os.path.join(args.log_dir, 'best_sat_model_resnet%d.pth' % args.seed))
            torch.save(sdt, os.path.join(args.log_dir, 'best_sat_model_sdt%d.pth' % args.seed))
        ...
        logger.write('Epoch [%d]:\tTrain loss: %.4f\tTrain cee_loss: %.4f\tTrain sym_loss: %.4f\tTrain sat: %.4f\t'
                ...
                'Best val epoch: %d\n' 'Best val sat epoch: %d\n'
                % (epoch, train_loss_avg,train_ccentropy_loss_meter.avg, train_sym_loss_metter.avg, train_sat_meter.avg, val_loss_avg, val_ccentropy_loss_meter.avg, val_sym_loss_metter.avg, val_sat_meter.avg, val_acc_meter.avg, best_val_epoch, best_sat_epoch)) 
        logger.flush()
        ...
        if epoch % args.save_step == 0:
            torch.save(resnet, os.path.join(args.log_dir, '%d_model_resnet.pth' % args.seed))
            torch.save(sdt, os.path.join(args.log_dir, '%d_model_sdt.pth' % args.seed))
...
def parse_arguments(experiment):
    ...
    parser.add_argument('exp', type=str,
                        choices=[ 'Joint_SoftDecisionTreeSymbolicFusionGuarded'],
