...
from MonuMAI.symbolic_utils import compute_accuracy,compute_sat_level,get_axioms_attr,get_axioms_classes, SatAgg, LogitsToPredicateResnetTree, LogitsToPredicateMulticlass,LogitsToPredicateMultilabel

def run_epoch(predicate, optimizer, loader, loss_meter, acc_meter, sat_meter, criterion, attr_criterion, args, is_training):
    """
    ...
    for batch_idx, data in enumerate(loader):           
        ...
        loss_symbolic = 1. - sat_agg  #loss_main = criterion(outputs, labels_var)  -> we could experiment with this
        losses.append(loss_symbolic)
        
        if is_training:
            losses[0] = losses[0]  + predicate.model.get_penalty()             
            attr_outputs = predicate.model.first_model.forward(inputs_var)  
        else:
            attr_outputs, outputs = predicate.model.forward(inputs_var,tree_penalty = False)     
        ...
        if is_training:           
            ...
            optimizer.step()
    if is_training:
        return loss_meter, sat_meter
    else:
        return loss_meter, sat_meter, acc_meter
...
def train(model, args):   
    ...
    for epoch in range(0, args.epochs):
        ...
        if args.no_img:
            ...
            train_loss_meter, train_sat_meter = run_epoch(model, optimizer, train_loader, train_loss_meter, train_acc_meter, train_sat_meter, criterion, attr_criterion, args, is_training=True)
       ...
       # evaluate on val set
        ...
        with torch.no_grad():
            val_loss_meter,val_sat_meter, val_acc_meter = run_epoch(model, optimizer, val_loader, val_loss_meter, val_acc_meter, val_sat_meter, criterion, attr_criterion, args, is_training=False)
        ...
        logger.write('Epoch [%d]:\tTrain loss: %.4f\tTrain sat: %.4f\t'
                'Val loss: %.4f\tVal sat: %.4f\tVal acc: %.4f\t'
                'Best val epoch: %d\n'
                % (epoch, train_loss_avg, train_sat_meter.avg, val_loss_avg, val_sat_meter.avg, val_acc_meter.avg, best_val_epoch)) 
        logger.flush()
...
def train_X_to_C_to_y_soft_decision_tree_symbolic(args):  
    
    resnet = resnet50(weights=ResNet50_Weights.DEFAULT)
    num_ftrs = resnet.fc.in_features
    resnet.fc = torch.nn.Linear(num_ftrs, args.n_attributes)
    model1 = resnet           
    model2 = SDT(input_dim=args.n_attributes,output_dim=N_CLASSES,depth=4,use_cuda=torch.cuda.is_available())
    logitTreeToP = LogitsToPredicateResnetTree(model1, model2)
    
    P = ltn.Predicate(logitTreeToP)
    
    train(P, args)
    
def train_X_to_C_to_y_soft_decision_tree_symbolic_2predicates(args):  
    
    resnet = resnet50(weights=ResNet50_Weights.DEFAULT)
    num_ftrs = resnet.fc.in_features
    resnet.fc = torch.nn.Linear(num_ftrs, args.n_attributes)
          
    P_concepts = LogitsToPredicateMultilabel(resnet)
    sdt = SDT(input_dim=args.n_attributes,output_dim=N_CLASSES,depth=4,use_cuda=torch.cuda.is_available())
    P_classifier = LogitsToPredicateMulticlass(sdt)
   
    logitTreeToP = LogitsToPredicateResnetTree(P_concepts.logits_model, P_classifier.logits_model)
    
    P_full = ltn.Predicate(logitTreeToP)
    
    train(P_full,P_concepts,P_classifier, args)
...
def parse_arguments(experiment):
    ...
    parser.add_argument('exp', type=str,
                        choices=[ 'Joint_SoftDecisionTreeSymbolic'],
